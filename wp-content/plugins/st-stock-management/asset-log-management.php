...
/**
 * Asset Logs Management - With Pagination
 */
...
// Add admin menu
add_action('admin_menu', 'asset_logs_admin_menu');

function asset_logs_admin_menu() {
    add_submenu_page(
        'stock-management',
        'Admin Asset Logs',
        'Admin Asset Logs',
        'manage_options',
        'asset-logs-management',
        'asset_logs_admin_page'
    );
}

// Function to format JSON data into readable format
function format_json_readable($json_string) {
...
function format_action_display($action) {
    $action_map = array(
        'created' => array(
            'label' => 'Asset Created',
        ...
        'asset_type_created' => array(
            'label' => 'Asset Created',
            'class' => 'action-created'
        ),
        'asset_type_updated' => array(
            'label' => 'Asset Updated',
            'class' => 'action-updated'
        ),
        'asset_type_deleted' => array(
            'label' => 'Asset Deleted',
            'class' => 'action-deleted'
        )
    );
...
function asset_logs_admin_page() {
    global $wpdb;
    ...
    <div class="wrap">
        ...
        $table_name = $wpdb->prefix . 'admin_item_stock_management_logs';
        
        // Check if table exists
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
            echo '<div class="error-notice">❌ Error: Asset logs table not found in database.</div>';
            return;
        }
        ...
        <div class="logs-container">
            <?php if (empty($logs)) : ?>
                <div style="text-align: center; padding: 40px; color: #6c757d;">
                    <p>No asset logs found in the database.</p>
                </div>
            <?php else : ?>
                <div class="tablenav">
                ...
                <table class="logs-table">
                    ...
                    <tbody>
                        <?php foreach ($logs as $log) : ?>
                            <?php $action_display = format_action_display($log->action); ?>
                            <tr>
                                <td><?php echo esc_html($log->id); ?></td>
                                <td><?php echo esc_html($log->asset_type_id); ?></td>
                                <td>
                                ...
                                <td>
                                    <?php echo format_json_readable($log->old_value); ?>
                                </td>
                                <td>
                                    <?php echo format_json_readable($log->new_value); ?>
                                </td>
                                <td>
                                    <?php 
